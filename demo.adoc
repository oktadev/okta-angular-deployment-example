...
. For `static.json` to be read, you have to use the https://github.com/heroku/heroku-buildpack-static[Heroku static buildpack]. This buildpack is made for SPA applications, so you can change the `scripts` section to Angular defaults. Add `--prod` to the `build` script since this is the script the static buildpack runs.
+
[source,json]
----
"scripts": {
  ...
  "headers": [
    {
      ...
      "value": "default-src 'self'; script-src 'self' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self' data:; frame-ancestors 'none'; connect-src 'self' https://*.okta.com"
    },
    {
      "key": "Referrer-Policy",
